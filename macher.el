...
(declare-function gptel-org-set-topic "gptel-org" (topic))
...
- ='org' - like ='default', but instead of the `gptel-default-mode',
  ...
  `gptel-default-mode', this will behave exactly the same as ='default'.
...
- nil - don't perform any action buffer setup.  The action buffer will
  ...
  gptel requests made through `macher-action'.  Use this if you want
  full control over the action buffer UI - you can customize the
...
Call function `macher-focus-string' interactively to yank this value."
...
- PRESET is the preset string or plist that will be applied.  See
  `macher-actions-alist' for allowed formats.
...
EXECUTION is a `macher-action-execution' struct.  This is part of the
default action buffer UI: it is added buffer-locally to
`macher-before-action-functions' by `macher--action-buffer-setup-ui'."
  (macher--apply-preset-locally (macher-action-execution-preset execution)))

(defun macher--before-action-scroll (_execution)
  "Scroll the action buffer window to show point.

When `macher--before-action-insert-prompt' runs, point is left at the
end of the just-inserted prompt.  This function updates the window to
show that position.
...
(defun macher--transform-backticks-to-org-src (text)
  "Transform backtick code blocks in TEXT to org src blocks.

Finds all ```language...``` blocks and converts them to
#+begin_src language...#+end_src blocks, properly escaping
the content with `org-escape-code-in-string'."
  (require 'org-src)
  (with-temp-buffer
    (insert text)
    (goto-char (point-min))
...
EXECUTION is a `macher-action-execution' struct.  This function
inserts the prompt text at the end of the buffer, adapting formatting
based on the major mode.  In `org-mode', it also adds a topic heading
and sets the GPTEL_TOPIC property so the conversation can be continued
via `gptel-send'.

After this function returns, point is at the end of the inserted
prompt text.  The buffer is not displayed or scrolled; use
`macher--before-action-display-buffer' and
`macher--before-action-scroll' for that.

This is added buffer-locally to `macher-before-action-functions' by
`macher--action-buffer-setup-basic'."
  (let* ((preset (macher-action-execution-preset execution))
         (prompt (macher-action-execution-prompt execution))
         (action (macher-action-execution-action execution)))

    (goto-char (point-max))

    ;; Add a topic heading when in org mode. This allows continuing the conversation for a single
    ;; action via normal gptel interactions.
    (when (derived-mode-p 'org-mode)
      ;; Make sure the topic heading is on its own line.
      (unless (bolp)
        (insert "\n"))
      ...
      (let* ((header-prefix "* ")
             ...
             (truncated-summary
              (let* ((lines (split-string summary "[\r\n]+" t "[[:space:]]*"))
                     ...
                     ;; Calculate available space: total fill-column minus prefix and postfix.
                     (used-length (+ (length header-prefix) (length header-postfix)))
             ...
             (topic
              (format "macher-%s-%s-%s"
                      ...
                      (replace-regexp-in-string "[[:space:]]+" "-" (downcase truncated-summary)))))
        ...
        (condition-case err
          ...
          (error
           (warn "Failed to set org topic: %S" err)))
        (goto-char (point-max))))
    ...
    ;; Insert presets via @-syntax if they exist in the global registry.
    (when (gptel-get-preset preset)
      (insert (format "@%s " preset)))

    ;; Insert the prompt text, transforming backtick blocks to org src blocks in org-mode.
    (let ((prompt-start (point))
          (final-prompt
           (if (derived-mode-p 'org-mode)
               (progn
                 (require 'org-src)
                 (macher--transform-backticks-to-org-src prompt))
             prompt)))
      (insert final-prompt)
      ;; Fold org src blocks after insertion.
      (when (derived-mode-p 'org-mode)
        (save-excursion
...
(defun macher--before-action-display-buffer (_execution)
  "Display the action buffer.

This is added buffer-locally to `macher-before-action-functions' by
`macher--action-buffer-setup-basic'."
  ;; Use a no-op buffer context so `display-buffer' doesn't change the selected buffer while we're
  ;; in the middle of running the before-action hooks.
...
Returns a string with information about the current buffer, including
file path, language, cursor position or selection, and relevant context
text."
...
IS-SELECTED (vs being entered manually)."
  (let ((focus-string (macher-focus-string)))
    (concat
     (when focus-string
       (concat
        macher--action-focus-prefix focus-string
        (unless (string-suffix-p "\n" focus-string)
          "\n")))
     (if is-selected
         "Implementation request (from user-selected text),"
       "Implementation request,")
     " use workspace tools to implement:\n\n" input)))
  ...
  (let* ((patch-buffer (or patch-buffer (macher-patch-buffer)))
    ...
    (concat
     ...
     (if (and input (not (string-empty-p input)))
         (concat "Revise your previous work based on these instructions:\n\n" input)
...
(defun macher--resolve-preset (preset)
  "Resolve PRESET to a plist spec.

If PRESET is a symbol, look it up from the global gptel registry or from
`macher-presets-alist'.  Otherwise return PRESET as-is."
  (if (symbolp preset)
      (or (gptel-get-preset preset) (cdr (assq preset macher-presets-alist)))
    preset))
...
CALLBACK takes no arguments."
  (let ((preset-for-gptel (macher--resolve-preset preset)))
    ...
    ;; WORKAROUND: gptel--preset-syms has a bug where (nconc syms ...) doesn't update syms when
    ;; it's nil, so parent-contributed symbols are silently dropped.  This means gptel-with-preset
    ;; won't save/restore variables set by parent presets.  We work around this by stripping
    ;; :parents and applying them via nested calls, so gptel-with-preset never sees a :parents key.
...
(defun macher--apply-preset-locally (preset)
  "Apply PRESET settings buffer-locally.
